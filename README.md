...
| `KWIN_FRAME_BREAKDOWN_DETAIL` | `0` | Per-effect drill-down for the prepaint and paint phases. Writes two sidecar files (`kwin perf prepaint detail <output>.csv` and `kwin perf paint detail <output>.csv`) with one row per spike frame: `frame_seq,wall_ns,phase,total_ns,scene_ns,details`, where `details` is `name1=ns1;name2=ns2;…` of exclusive per-effect times sorted descending. Also writes `kwin perf offscreenquick detail (default).csv` — one row per `OffscreenQuickView::update()` call that crosses the threshold, with columns `wall_ns,view_size,total_ns,polish_ns,beginframe_ns,sync_ns,render_ns,endframe_ns` — to attribute QuickSceneEffect (Overview, Window-View, …) cost to a specific QML render-control stage. Independent from `KWIN_FRAME_BREAKDOWN`. |
| `KWIN_FRAME_BREAKDOWN_DETAIL_THRESHOLD_MS` | `1` | Sidecar rows are written only when the phase's total time (scene + effects) is at or above this threshold in milliseconds. Set to `0` to log every frame. |
