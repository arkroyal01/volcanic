...
void SurfacePixmapX11::create()
{
    ...
    if (xcb_generic_error_t *error = xcb_request_check(connection, namePixmapCookie)) {
        qCWarning(KWIN_CORE, "Failed to create window pixmap for window 0x%x (error code %d)",
    ...
    if (!windowAttributes || windowAttributes->map_state != XCB_MAP_STATE_VIEWABLE) {
        qCWarning(KWIN_CORE, "Failed to create window pixmap for window 0x%x (not viewable)",
                  window->window());
        xcb_free_pixmap(connection, pixmap);
        return;
    }
    const QRectF bufferGeometry = window->bufferGeometry();
    if (windowGeometry.size() != Xcb::toXNative(bufferGeometry.size())) {
        qCWarning(KWIN_CORE, "Failed to create window pixmap for window 0x%x: window size (%dx%d) != buffer size (%fx%f)", window->window(),
