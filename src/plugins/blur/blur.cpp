...
BlurEffect::~BlurEffect()
{
    ...
    // Destroy the shared blur helper before the context disappears (its
    // pipelines/render pass belong to m_vulkanCtx).
    m_kawaseBlur.reset();
#endif
}
...
bool BlurEffect::initVulkanResources()
{
    ...
    // Shared dual-kawase blur: linear sampler, descriptor + pipeline layout
    // (12-byte fragment push constants), the GENERAL→GENERAL intermediate
    // render pass, and the down/up pipelines. The composite pipeline below
    // reuses its descriptor set + pipeline layout to blend the result onto
    // the swapchain.
    m_kawaseBlur = VulkanKawaseBlur::create(m_vulkanCtx, kBlurFmt);
    if (!m_kawaseBlur) {
        return false;
    }

    // Shader modules (the down/up modules live inside the helper; here we
    // only need the shared full-screen vertex shader and the composite frag).
    ...
    // Composite pipeline. It targets the swapchain via m_vulkanCompositePass
    // (created below) and enables constant-alpha blending so the blur can
    // crossfade with the existing scene during opacity animations (matches
    // the GL contrast pass). It binds the shared blur descriptor + pipeline
    // layout, so its only bespoke parts are the fragment shader and blend.
    auto makePipeline = [&](VkShaderModule fragMod,
        ...
        pipeInfo.layout = m_kawaseBlur->pipelineLayout();
                            ...
                            EffectWindow *w, int mask, const QRegion &region, WindowPaintData &data)
{
    ...
    if (vkData.textures.size() != neededTextures
        ...
        for (size_t i = 0; i <= m_iterationCount; ++i) {
            ...
            auto fbTex = VulkanFramebuffer::create(m_vulkanCtx, m_kawaseBlur->renderPass(),
                                                   tex->imageView(), sz);
            if (!fbTex) {
    ...
    // Helper: bind source texture as descriptor set (used by the composite
    // pass below; the kawase pyramid binds its sources internally). Uses the
    // shared blur descriptor + pipeline layout.
    auto bindSrc = [&](VulkanTexture *src) {
        ...
        m_vulkanCtx->bindDescriptors(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                     m_kawaseBlur->pipelineLayout(),
                                     m_kawaseBlur->descriptorSetLayout(),
                                     0, 1, &wr);
    };

    // Run the shared dual-kawase pyramid over tex[0..N]. tex[0] is the
    // captured swapchain region (just blitted, made visible above) and the
    // final destination; the down/up chain reads/writes the rest. Y-flipped
    // viewport because the capture is in top-down swapchain orientation. The
    // trailing barrier recordPyramid inserts leaves tex[0] visible to the
    // composite's shader read.
    std::vector<VulkanKawaseBlur::Level> levels;
    levels.reserve(vkData.textures.size());
    for (const auto &tex : vkData.textures) {
        levels.push_back({tex->imageView(), nullptr, QSize(tex->width(), tex->height())});
    }
    for (size_t i = 0; i < vkData.framebuffers.size(); ++i) {
        levels[i].framebuffer = vkData.framebuffers[i].get();
    }
    m_kawaseBlur->recordPyramid(cmd, levels, static_cast<float>(m_offset), /*flipViewportY=*/true);
